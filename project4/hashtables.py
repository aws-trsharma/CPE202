...
class HashTableLinear:
    ...
    def put(self, key, data):
        ...
        if self.load_factor() <  0.75:
            ...
            elif self.list[val] is not None and self.list[val][0] == key:
                self.list[val] = key, data
            else:
    ...
    def key(self):
        """
        Get keys in the table to extract all key-value table
        :return:
        """
        keys = []
        item = 0
        ...
        while item < size:
            if self.list[item] is None:
                item += 1
            else:
                keys.append(self.list[item][0])
                item += 1
        return keys
...
class HashTableQuadratic:
    ...
    def key(self):
        """
        Returns the key associated with the key-value pair in list format
        :return:
        """
        keys = []
